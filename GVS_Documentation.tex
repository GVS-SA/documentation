...
\begin{document}
	...
	\noindent Während dem Projekt konnten eine erweiterbare Software Architektur erstellt werden, die das Vorgängerprodukt vollumfänglich ersetzt. Zusätzlich wurde das Usability verbessert und durch den Einsatz von speziellen Layouting Algorithmen eine verbesserte/ansprechende Darstellung von ''Graphen'' und ''Trees'' erreicht.
	
	%TODO Replay funtionalität
	...
	GVS ist eine Visualisierungs-Software die Graphen und Trees ansprechend darstellt. Die Datenstrukturen werden in der Entwicklungsumgebung des Benutzers programmiert und mit einer clientseitingen Library an die Visualisierungskomponenten gesendet. Die Server Komponenten interpriert die eingehenden Daten und stellt diese auf eine ansprechende Art und Weise dar. \\
	...
	In einer ersten Phase wird die bestehende Software auf Funktionsweise und Verbesserungspotential analysiert. Da die Software HSR Intern verwendet wird, sind Folgearbeiten nicht auszuschliessen. Es wird deshalb darauf geachtet, dass mit den gewonnen Erkenntnissen eine Architektur designed wird, die einfach verständlich und erweiterbar ist. Ebenfalls sollen zeitgemässe Software Entwicklungs Techniken angewendet werden. \\
	...
	In der Studienarbeit wurde ein vollumfänglicher Ersatz für den GVS 1.0 entwickelt. GVS 2.0 erweitert sogar die Funktionalität um ein intuitiveres User Interface sowie einer verbesserte Darstellung der Datenstruturen.
	
	\subsection*{Ausblick}
	Das Projektteam ist vom praktischen Nutzen der Applikation überzeugt und wir sich deshalb in der Bachelorarbeit mit einer erweiterten Anwendung beschäftigen. Diese wird den Studenten das Erlenenen von weiteren theoretischen Informatikkonzepten durch Visualisierung erleichtern.  
	...
	\begin{description} \label{desc:architecture-changes}
		...
		In der Designphase wurde das Zusammenführen von Graphen und Trees angedacht. In der Realisierung hat sich aber gezeigt, dass einerseits die Layouter sowie infolgedessen auch die Vertices unterschieden werden müssen. (siehe \ref{graph-vs-tree}) Für den Tree Layouter (\ref{ssec:treelayouter}) wird deshalb neben dem \textit{TreeVertex} auch ein \textit{LeafVertex} benötigt.  Blattknoten dienen als Platzhalter und werden nicht dargestellt. Sie vereinfachen die Berechnung des Tree Layouts aber stark.
	\end{description}
	...
	Details zu den genauen Änderungen gegenüber GVS 1.0 finden sich im Abschnitt \ref{label}. %TODO: ergänzen welcher Abschnitt, wenn klar ist, wie Realisations Kapitel strukturiert ist, änderungen sind watchdog, clientConnection&gvsxmlreader,...
