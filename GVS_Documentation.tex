...
\begin{document}
	...
	Die Software \gls{gvs} unterstützt Studenten beim Erlernen von schwierig zu verstehenden Algorithmen. Der wichtigste Aspekt ist dabei die Visualisierung der einzelnen Arbeitsschritte, die ein Algorithmus auf einem Graph oder Tree ausführt.\\
	...
	\subsection{Mehrwert}
	Die Software profitiert von einer durchgängig klaren Architektur, die einfach erweitert werden kann. Der Endbenutzer bemerkt nur eine modernes und ansprechendes User Interface. Die Funktionalität soll beibehalten werden. 
	...
	\begin{enumerate}
		...
		\item Clientseitig muss der Java und .NET Client um Generics erweitert werden. 
		\item Punktuell müssen Verbesserungen vorgenommen werden.
	\end{enumerate}

	Welche Punktuellen Verbesserungen vorgenommen werden sollen, hat das Projekttem zusammen mit dem Betreuer, Herr Letsch, festegelegt und sind dem Abschnitt \textbf{Refactroing Ablauf} (\ref{ssec:ablauf}) zu entnehmen.
	
	\section{Domainanalyse}
	In der Domainanalyse hat sich das Projektteam einen Überblick über den aktuellen Stand von GVS 1.0 verschafft. Die dabei gewonnen Erkenntnisse sind dann in die Designspezifikation eingeflossen.
	
	\subsection{Stakeholder}
	Aus den beschrieben Anforderungen kristallisieren sich drei relevante Stakeholder heraus.
	\begin{table}[h!]
		\centering
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\linewidth]{assets/images/gvs-component}
		\caption[Komponentendiagramm]{Komponentendiagramm}
		\label{fig:gvs-component}
	\end{figure}
	...
	\begin{description}
		\item[GVS User] \hfill \\
		Der User interagiert mit dem GVS Server über das User Interface. Über das User Interface kann er die aktuelle Session auf das Filesystem speichern oder eine Session von dort laden. Wenn ein Graph visualisiert ist, kann er die Koordinaten der Vertices neu berechnen lassen.
		\item[GVS Client Lib] \hfill \\
		Die Client Lib wird in das Software Projekt des GVS User eingebunden. Sie sendet in \gls{xml} beschriebene Graphen über eine Socket Verbindung.
	\end{description}
	
	
	\subsection{Klassendiagramm} \label{ssec:klassendiagramm-1}
	Das Klassendiagramm gab dem Projektteam eine gute Übersicht über die Struktur des GVS 1.0. Die Klassen wurden so organisiert, dass sie einer der drei Schichten ''Presentation'', ''Business'' und ''Access'' zugewiesen werden konnten. Bei der Analyse sind dem Projektteam einige Punkte aufgefallen, die im Abschnitt Erkenntnisse (\ref{sec:erkenntnisse}) genauer beschrieben sind.
	...
	Aus der Analysephase konnten wertvolle Erkennisse für das weitere Vorgehen gewonnen werden.
	
	\subsection{Stärken}
	Der GVS 1.0 weisst folgende Stärken auf:
	
	\subsubsection{Trennung Socket / Visualization}
	Die angedachte Trennung zwischen Server und Visualisierung der Daten wiederspiegeln sich klar im Code.
	
	\subsubsection{Layering}
	Die meisten Klassen konnten bei Analyse klar einem bestimmten Layer zugewiesen werden. Dies zeigt, dass die angedachte Architektur mehrheitlich umgesetzt wurde.
		
	\subsection{Schwächen}
	Der GVS 1.0 weisst folgende Schwächen auf:
	
	\subsubsection{Swing/AWT Imports im Business Layer} Im Business Layer wurden diverse Imports der UI Frameworks \gls{swing} rsp. \gls{awt} gefunden. Dies erschwert einen Austausch des eingesetzten UI Frameworks.
	
	\subsubsection{Tangles} Zwischen den Software Schichten gibt es Zugriffe unten nach oben.
	...
	Eine Prüfung der Namenskonvention wurde offenbar nicht durchgeführt. Zum Beispiel sind volatile Variablen in Grossbuchstaben geschrieben, was normalerweise auf eine Konstante hindeutet.
	...
	Mehrere Klassen sind mehr als 500 Zeichen lang. Dies hebt den Verdacht, dass die Klassen mehr als eine Aufgabe erledigen.
	
	\subsubsection{Keine Unit Tests} In dem Projekt wurden keine Unit Tests geschrieben. Dies ruht vermutlich daher, dass JUnit zum Entwicklungszeitpunkt noch sehr jung war.
	
	\subsubsection{JavaDoc}
	Das Verhalten von Klassen nicht durchgehend nachgeführt. Ebenfalls lässt die Qualität der Kommentare zu wünschen übrig.	
	
	
	\subsection{Schlüsse}
	Aus den gewonnen Erkenntnissen hat das Projektteam Metriken eingeführt, damit der neue Code auf Konventionen Konformität geprüft werden kann. Ebenfalls wurde vor dem Start mit der Releasierung erste Refactorings durchgeführt, um den alten Code an die neuen Metrik Konventionen anzugleichen.
	...
	Das neue Klassendiagramm orientiert sich stark an die Vorgängerversion des GVS 1.0. (Siehe \ref{ssec:klassendiagramm-1}). Einzig der Presentation Layer wurde komplett durch die neu eingeführte \gls{mvvm} Struktur ersetzt. (Siehe \ref{sssec:mvvm})
	...
	\begin{enumerate}
		\item Das Presentation Layer wird ersetzt. Dabei wird \gls{swing} durch JavaFX ersetzt. Mehr Details finden sich in der Klassen-Refactoring-Liste (\ref{ssec:class-refactroing-list}) sowie im Klassendiagramm von GVS 2.0 (\ref{sec:class-diagram})
	...
	\subsection{Klassen Refactoring Liste}
