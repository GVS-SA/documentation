...
\begin{document}
	...
	\begin{tabularx}{\linewidth}{l l X l}
		...
		08.12.17 & 1.0 & Multithreading beschrieben & mwieland \\
		\bottomrule 
	\end{tabularx} 
	...
	\section{Refactoring Layout Engine}
	
	\subsection{LayoutController} \label{ssec:layoutcontroller}
	Der \textit{LayoutController} steuert die Berechnung der Koordinaten durch die Physik Engine. Sobald ein Graph gelayouted werden muss, wird ein \textit{AreaTicker} Thread gestartet, der in einem bestimmten Interval das Anzeigen der neuen Koordinaten veranlasst. Es läuft immer nur genau ein \textit{AreaTicker} Thread. Sobald sich die \textit{Particle} in der \textit{Area} stabilisiert haben, wird der \textit{AreaTicker} Thread terminiert. 
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\linewidth]{assets/images/sequence_layoutcontroller}
		\caption{Sequenzdiagramm LayoutController}
		\label{fig:sequencelayoutcontroller}
	\end{figure}
	
	\subsection{Area} \label{ssec:area}
	Die \textit{Area} dient als Container während dem dynamischen Layoutprozess. Sie beinhaltet mehrere \textit{Particle}s die von physischen Kräften bewegt werden. (\textit{Traction}s und \textit{RepulsiveForce}s) Ein \textit{Particle} entspricht einem Knote des Graphen oder Trees. Die Positionen der \textit{Particles} werden fortlaufend berechnet. Sobald ein Tick Event passiert werden die berechneten Koordinaten des Particles in den Business Vertex kopiert. Über eine Observer Beziehung wird das \textit{VertexViewModel} im Presentation Layer über die neuen Koordinaten notifziert. Dessen \gls{javafx} Properties sind über ein bidirektionales Binding mit dem UI \textit{Circle} verbunden. Dieser aktualisiert dann automatisch und visualisiert die Änderungen auf dem UI.
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\linewidth]{assets/images/sequence_area}
		\caption{Sequenzdiagramm Area}
		\label{fig:sequencearea}
	\end{figure}

	\section{Multithreading} \label{sec:threading}
	Um die Oberfläche reaktiv zu halten und gleichzeitig auf Datenübertragungen von der Client Lib zu reagieren werden mehrere Threads benötigt. Daher müssen Synchronisationspunkte eingerichtet werden, damit typische Gefahren der Nebenläufigkeit (z.B Race Conditions) nicht auftreten können. Im GVS 2.0 sind folgende Klassen als eigenständige Threads konzipiert.
	...
	\begin{table}[h!]
		...
		\begin{tabularx}{\linewidth}{l X X}
			...
			\textit{ClientConnection} & Interpretiert die eingehende Verbindung und leitet die XML Daten an den ModelBuilder weiter & Während einer Client Verbindung \\
			\textit{Watchdog} & Verhindert dass ein Client den Server nur so lange reserviert wie auch Daten übertragen werden & Während einer Client Verbindung \\
	...
	\begin{table}[h!]
		...
		\begin{tabularx}{\linewidth}{l X X}
			...
			\textit{GraphLayouter} & Überwacht die Lebensdauer des TickerThread & Programmstart bis App Terminierung. \\
			\bottomrule 
		\end{tabularx} 
		\caption{Übersicht nebenläufige Klassen} 
	\end{table}
	...
	Wichtig ist, dass alle Änderungen am User Interface ausschliesslich vom JavaFX UI Thread durchgeführt werden. Dazu werden bei allen Eintrittspunkten in den Presentation Layer ein \lstinline|runLater()| ausgeführt. Dies veranlasst einen Thread Wechsel zum UI Thread. 
