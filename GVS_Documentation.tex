...
\begin{document}
	...
	Der Presentation Layer ist entkoppelt von der restlichen Anwendung, da er oft technologiespezifische Komponenten enthält. (z.B \gls{javafx}). Er ist nach dem \gls{mvvm} Prinzip organisiert.
	...
	\paragraph{View}
	Die View stellt den UI Zustand des View Model dar. Sie setzt sich aus einer Controller Klasse sowie eine \gls{fxml} Datei zusammen. Die View beinhaltet keine nennenswerte Logik und leitet die eingehenden Request einfach an das View Model weiter. Die Weiterleitung wird mit der Databinding-API gemacht. 
	
	\paragraph{View Model}
	Das ViewM Model enthält den gesamten UI Zustand. (z.B Button A ist aktiv). Es kennt die View nicht und hat keine Abhängigkeiten zu konkreten View Elementen.  Dies hat den grossen Vorteil, dass die gesamte UI Logik im View Model gekapselt ist und somit sehr gut testbar ist.
